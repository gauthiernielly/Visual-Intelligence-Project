...
JSON_PATH = "../results/hybrid_pipeline_results.json"
...
def get_json_sequence(json_data: dict, video_id: str) -> list:
    ...
    # We collapse consecutive duplicates
    collapsed_seq = []
...
def lcs_traceback(gt_seq, pred_seq):
    """Returns the precise Ground Truth indices that were successfully matched."""
    m, n = len(gt_seq), len(pred_seq)
    dp = [[0] * (n + 1) for _ in range(m + 1)]
    ...
    # Traceback to find which GT items matched
    matched_gt_indices = []
    i, j = m, n
    while i > 0 and j > 0:
        if gt_seq[i - 1] == pred_seq[j - 1]:
            matched_gt_indices.append(i - 1)
            i -= 1
            j -= 1
    ...
    return dp[m][n], matched_gt_indices[::-1]
...
def main():
    ...
    for video_id in video_ids:
        ...
        if gt_seq and len(gt_seq) > 0:
            ...
            # 2. Graph Metrics (Traceback)
            _, matched_indices = lcs_traceback(gt_seq, pred_seq)
            video_lengths.append(len(gt_seq))
            video_recalls.append(recall)
            
            for idx, action in enumerate(gt_seq):
                class_totals[action] += 1
                if idx in matched_indices:
                    class_matches[action] += 1
    ...
    # Graphs
    print("\nGenerating visual plots for presentation...")
    
    # Plot 1: Recall Per Class
    class_recall_data = []
    for cls, total in class_totals.items():
        if total >= 5: # We only plot classes with at least 5 occurrences for statistical relevance
            recall_pct = (class_matches[cls] / total) * 100
            class_recall_data.append({
                "Action": cls.replace("_", " ").title(), 
                "Recall (%)": recall_pct
            })
            
    df_classes = pd.DataFrame(class_recall_data).sort_values(by="Recall (%)", ascending=False)
    
    plt.figure(figsize=(14, 10))
    sns.set_theme(style="whitegrid")
    sns.barplot(data=df_classes, x="Recall (%)", y="Action", palette="viridis")
    plt.title("Action Recognition Recall by Class (Min. 5 occurrences, 68 Videos)", fontsize=16, fontweight='bold')
    plt.xlabel("LCS Recall (%)", fontsize=12)
    plt.ylabel("Action Class", fontsize=12)
    plt.tight_layout()
    plt.savefig("recall_per_class.png", dpi=300)
    print("Saved 'recall_per_class.png'")

    # Plot 2: Video Length vs Recall
    plt.figure(figsize=(10, 6))
    sns.regplot(
        x=video_lengths, 
        y=video_recalls, 
        scatter_kws={'alpha':0.6, 's':80, 'color':'#0984e3'}, 
        line_kws={'color':'#d63031'}
    )
    plt.title("Pipeline Robustness: Video Length vs. Recall Score (68 Videos)", fontsize=14, fontweight='bold')
    plt.xlabel("Complexity (Total Ground Truth Actions in Video)", fontsize=12)
    plt.ylabel("Video Recall Score (%)", fontsize=12)
    plt.ylim(0, 105)
    plt.tight_layout()
    plt.savefig("recall_vs_length.png", dpi=300)
    print("Saved 'recall_vs_length.png'")
