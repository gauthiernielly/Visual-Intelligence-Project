"""
Shared evaluation script for the TAD, VLM and Hybrid pipelines on TSU.

Reads any predictions JSON in the {"results": {video_id: [{segment, label, score}]}}
schema and a ground-truth source (either a directory of per-video CSVs with
columns start_frame/end_frame/event, or an ActivityNet-style JSON) and reports
LCS Recall/Precision/F1, segment-level mAP at IoU 0.1/0.3/0.5, per-class AP at
IoU 0.5, class coverage, and a per-subject breakdown.

Two systems can be scored in one call by passing two --pred arguments together
with --names, in which case a comparison summary is printed at the end.
"""
...
def load_gt_from_csv_dir(
    ...
    for video_id in video_ids:
        ...
        try:
            ...
            # Derive timestamps. Prefer explicit columns, fall back to frame/fps.
...
def compute_map(
    ...
    for iou_thresh in iou_thresholds:
        for cls in all_classes:
            ...
            # AP via 11-point interpolation (VOC style)
            ap = 0.0
...
def evaluate(
    ...
    if os.path.isdir(gt_source):
        ...
        # Filter to only videos present in predictions
